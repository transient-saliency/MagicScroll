...
class MultiDiffusion(nn.Module):
    ...
    def prepare_latents(self, batch_size, num_channels_latents, image, timestep, height, width, dtype, device,
        ...
        # noise3 = randn_tensor(shape, generator=generator, device=device, dtype=dtype)
        # init_latents = self.scheduler.add_noise(init_latents, noise3, timestep)

        if latents is None:
            latents = init_latents
        else:
            latents = latents.to(device)
        print("init_latents_shape2:", init_latents.shape)  # -----------------------------------
    ...
    @torch.no_grad()
    def generate(self, fg_masks, fg_prompts, mg_masks, mg_prompts, image, fg_negative_prompts='',
                 mg_negative_prompts='',
        ...
        views = get_multi_scale_views(height, width, window_sizes, strides)
        ...
        with torch.autocast('cuda'):
            ...
            for i, t in enumerate(self.scheduler.timesteps):
                ...
                if i > bootstrapping_1:
                    # 前景去噪
                    value, count = self.denoise(i, t, views, fg_masks, latent, fg_prompts, background_prompts,
                                                       bootstrapping_backgrounds_1, bootstrapping_1, width, fg_noise,
                                                       text_embeds_fg, guidance_scale, value, count)
                else:
                    # 中景去噪
                    value, count = self.denoise(i, t, views, mg_masks, latent, mg_prompts, background_prompts,
                                                        bootstrapping_backgrounds_2, bootstrapping_2, width, mg_noise,
...
# Load and preprocess masks as per your code
...
def process_data_point(index, text, H, W, ref_image_path, device):
    ...
    print(text)
    # Your model output as per your code...（这个地方fg_bg_masks和fg_neg_prompts要比fg_prompts大1，mg_bg_masks和mg_neg_prompts要比mg_prompts大1，
    # 因为我们前面在proprocess的时候fg对全图并不是密铺，所以需要额外增加一个背景，但是由于mg是对全图，所以其实并不需要加一个额外的背景）
    # 所以这里处理的方式就是mg按照小的，fg按照大的
    print(len(fg_bg_masks))
    print(len(fg_prompts))
    print(len(mg_bg_masks))
    print(len(mg_prompts))
    print(len(fg_neg_prompts))
    print(len(mg_neg_prompts))
    model_output = model.generate(
        fg_bg_masks, fg_prompts, mg_bg_masks[1:], mg_prompts,
        init_image, fg_neg_prompts, mg_neg_prompts[1:],
...
if __name__ == '__main__':
    ...
    for i in range(0, 1):  # Starts the loop at i=3
        ...
        model_file_path = os.path.join(save_dir, f'model_output_{i}.png')
