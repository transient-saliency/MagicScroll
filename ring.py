...
#             w_start = int((i % num_blocks_width) * stride)
#             w_end = w_start + window_size
#             multi_scale_views.append((h_start, h_end, w_start, w_end))
...
def get_multi_scale_views(panorama_height, panorama_width, window_sizes=[64], strides=[8]):
    ...
    # 第二组视图：所有横坐标都是第一组视图的横坐标加上 panorama_width // 2
    offset_width = panorama_width // 2
    for view in multi_scale_views_1:
        h_start, h_end, w_start, w_end = view
        new_w_start = w_start + offset_width
        new_w_end = w_end + offset_width
        multi_scale_views_2.append((h_start, h_end, new_w_start, new_w_end))
...
class MagicScroll(nn.Module):
    ...
    @torch.no_grad()
    def prepare_latents(self, batch_size, num_channels_latents, images, coordinates, timestep, height, width, dtype, device,
                        generator=None, latents=None):
        # 预处理并编码所有图片
        preprocessed_images = [preprocess(image).to(device=device, dtype=dtype) for image in images]
        
        # 编码所有图片并调整通道数
        preprocessed_latents = []
        for preprocessed_image in preprocessed_images:
            latent = self.vae.encode(preprocessed_image).latent_dist.sample(generator)
            latent = self.vae.config.scaling_factor * latent
            
            if latent.shape[1] != num_channels_latents:
                if latent.shape[1] == 3:
                    alpha_channel = torch.ones((latent.shape[0], 1, latent.shape[2], latent.shape[3]), device=device, dtype=dtype)
                    latent = torch.cat((latent, alpha_channel), dim=1)
                else:
                    raise ValueError(f"Latent has {latent.shape[1]} channels, expected {num_channels_latents} channels.")
            
            preprocessed_latents.append(latent)
        
        # 创建一个空的 latent 图
        latent_height, latent_width = height//8, width//8
        init_latents = torch.zeros((batch_size, num_channels_latents, latent_height, latent_width), device=device, dtype=dtype)
        ...
        print("init_latents_shape_1:", shape)

        for latent, (x, y) in zip(preprocessed_latents, coordinates):
            # 将每张图片的 latent 插入到大 latent 图中的指定位置
            print("latents_shape:", latent.shape)
            init_latents[:, :, y:y+latent.shape[2], x:x+latent.shape[3]] = latent
        ...
        # 添加噪声
        noise2 = randn_tensor(shape, generator=generator, device=device, dtype=dtype)
        init_latents = self.scheduler.add_noise(init_latents, noise2, timestep)

        # 根据调度器的要求缩放初始噪声
        init_latents = init_latents * self.scheduler.init_noise_sigma
        print("init_latents_shape_2:", init_latents.shape)
        return init_latents
    ...
    # 生成总指令
    @torch.no_grad()
    def generate(self, fg_masks, fg_prompts, mg_masks, mg_prompts, images, fg_negative_prompts='',
                 mg_negative_prompts='',
        ...
        text_embeds_fg = self.get_text_embeds_compel(fg_prompts, fg_negative_prompts)  # Text embeddings for fg
        embed_dim_fg = text_embeds_fg.shape[1]
        left_half_fg = text_embeds_fg[:, :embed_dim_fg // 2]
        text_embeds_fg = torch.cat([text_embeds_fg, left_half_fg], dim=1)
        # text_embeds_mg = self.get_text_embeds(mg_prompts, mg_negative_prompts)  # Text embeddings for mg
        # print("text_embeds_mg",text_embeds_mg.shape)
        text_embeds_mg = self.get_text_embeds_compel(mg_prompts, mg_negative_prompts)
        embed_dim_mg = text_embeds_mg.shape[1]
        left_half_mg = text_embeds_mg[:, :embed_dim_mg // 2]
        text_embeds_mg = torch.cat([text_embeds_mg, left_half_mg], dim=1)
        print("text_embeds_mg", text_embeds_mg.shape)  # ------------------------------------------------------------------
        ...
        if use_ref_img:
            latent = self.prepare_latents(
                ...
                height,
                width,
                text_embeds_mg[0].dtype,
                device
            )
        else:
            latent = torch.randn((1, self.unet.in_channels, height // 8, width // 8), device=self.device)

        latent = torch.cat([latent, latent[:, :, :, :width//2]], dim=3)
        fg_masks = torch.cat([fg_masks, fg_masks[:, :, :, :width//2]], dim=3)
        mg_masks = torch.cat([mg_masks, mg_masks[:, :, :, :width//2]], dim=3)

        fg_noise = latent.clone().repeat(len(fg_prompts) - 1, 1, 1, 1)
        mg_noise = latent.clone().repeat(len(mg_prompts) - 1, 1, 1, 1)
        ...
        with torch.autocast('cuda'):
            ...
            for i, t in enumerate(self.scheduler.timesteps):
                ...
                if i < num_inference_steps-10:
                    if i % 2 == 0:
                        latent[:, :, :, :width//16] = latent[:, :, :, width//8:(width//8+width//16)]
                    else: 
                        latent[:, :, :, width//8:(width//8+width//16)] = latent[:, :, :, :width//16]

                if i > bootstrapping_1:
                    # 前景去噪
                    if i % 2 == 0:
                        value, count = self.denoise_interp(i, t, views_1, fg_masks, latent, fg_prompts, background_prompts,
                                                        bootstrapping_backgrounds_2, bootstrapping_2, width, fg_noise,
                                                        text_embeds_fg, guidance_scale, value, count)
                        value, count = self.denoise_interp(i, t, views_2, fg_masks, latent, fg_prompts, background_prompts,
                                                        bootstrapping_backgrounds_2, bootstrapping_2, width, fg_noise,
                                                        text_embeds_fg, guidance_scale, value, count)
                    else:
                        value, count = self.denoise_interp(i, t, views_2, fg_masks, latent, fg_prompts, background_prompts,
                    ...
                    # 中景去噪
                    if i % 2 == 0:
                        value, count = self.denoise_interp(i, t, views_1, mg_masks, latent, mg_prompts, background_prompts,
                                                        bootstrapping_backgrounds_1, bootstrapping_1, width, mg_noise,
                                                        text_embeds_mg, guidance_scale, value, count)
                        value, count = self.denoise_interp(i, t, views_2, mg_masks, latent, mg_prompts, background_prompts,
                                                        bootstrapping_backgrounds_1, bootstrapping_1, width, mg_noise,
                                                        text_embeds_mg, guidance_scale, value, count)
                    else:
                        value, count = self.denoise_interp(i, t, views_2, mg_masks, latent, mg_prompts, background_prompts,
...
## 单张图执行命令
def process_data_point(index, text, H, W, ref_image_path, device):
    use_ref_img = True
    seed = 128
    steps = 30
    bootstrapping_1 = 15
    ...
    init_images = load_images_from_folder(ref_image_path, W, H)
    ...
    fg_bg_masks, mg_bg_masks = load_and_preprocess_masks(index+1, H, W, device)

    # Read fg_prompts from the object_color.txt file
    # fg_prompts = read_fg_prompts(i+1)
    # print(fg_prompts)

    # Read mg_prompts from the Excel row
    # mg_prompts = [f'{prompt} in the style of {style_prompt}' for prompt in read_mg_prompts(row)]
    # print(mg_prompts)

    # Assuming the fg_prompts and mg_prompts are related to the masks
    # Get the list of all files in mask_output_fg_dir
    fg_file_paths = [os.path.join(mask_output_fg_dir, file) for file in os.listdir(mask_output_fg_dir) if file.endswith('.png')]

    # Now create fg_prompts using the list of file_paths
    fg_prompts = bg_prompt + [f'{os.path.splitext(os.path.basename(path))[0]}' for path in fg_file_paths]
    print(fg_prompts)

    mg_file_paths = [os.path.join(mask_output_mg_dir, file) for file in os.listdir(mask_output_mg_dir) if file.endswith('.png')]
    mg_prompts = [f'{os.path.splitext(os.path.basename(path))[0]} in the style of {style_prompt}' for path in mg_file_paths]
    print(mg_prompts)

    # mg_prompts = bg_prompt + [f'scene {i} in the style of {style_prompt}' for i in range(len(mg_bg_masks)-1)]  # -1 to exclude the background mask
    # print(mg_prompts)

    # Construct negative prompts similarly
    fg_neg_prompts = [bg_negative] + [fg_negative] * (len(fg_bg_masks)-1 )  # Adjust as necessary
    mg_neg_prompts = [bg_negative] + [mg_negative] * (len(mg_bg_masks)-1 )  # Adjust as necessary

    # Assuming fg_masks and fg_prompts are already defined
    boxes = []
    phrases = mg_prompts[0:]  # Excluding the background prompt which is at fg_prompts[0]

    for mask in mg_bg_masks[1:]:  # Excluding the background mask which is at fg_masks[0]
        ...
        if len(boxes) == boxes_before:
            # No new boxes were added for this mask, so add a (0,0,0,0) box
            boxes.append([0, 0, 0, 0])

    mg_bg_masks_pil = to_pil_image(mg_bg_masks[0].squeeze(0).cpu())

   
    model_output = model.generate(
        fg_bg_masks, fg_prompts, mg_bg_masks[1:], mg_prompts,
        init_images, fg_neg_prompts, mg_neg_prompts[1:],
...
if __name__ == '__main__':
    # Load the data
    data = pd.read_excel('./Chinese_Painting_Mask/data_description.xlsx', header=None)
    device = torch.device('cuda')
    sd_version = '2.1'

    # Base directory for the mask outputs
    base_dir = './Chinese_Painting_Mask/mask_output_'
    ...
    repo_id_embeds_1 = "sd-concepts-library/wu_shi"
    token_name_1 = "<wu_shi>"
    model.load_textual_inversion(repo_id_embeds_1,token_name_1)

    ref_image_path='./layout'
    ...
    for i in range(0, 30):  # Starts the loop at i=3
        row = data.iloc[i]  # Gets the row at index i

        print(i)
        mask_output_fg_dir = f'{base_dir}{i+1}/mask_output_fg{i+1}'
        mask_output_mg_dir = f'{base_dir}{i+1}/mask_output_mg{i+1}'

        # Check if the directories are empty
        if not os.path.isdir (mask_output_fg_dir) or not os.path.isdir (mask_output_mg_dir) or not os.listdir(mask_output_fg_dir) or not os.listdir(mask_output_mg_dir):
            print(f'Skipping iteration {i} as one or both mask_output directories are empty.')
            continue

        result = process_data_point(i, row.iloc[6], 512, 2048, ref_image_path, device)
        results.append(result)

        model_output = result['model_output']

        model_file_path = os.path.join(save_dir, f'test_{i}.png')

        model_output.save(model_file_path)